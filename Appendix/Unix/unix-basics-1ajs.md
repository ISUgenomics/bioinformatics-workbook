...
Copy and paste the following text into a file named myFirstFilEE.txt
```
...
Start by copying the text above using your mouse then in a terminal use nano to create a file named myFirstFilEE.txt

```
nano myFirstFilEE.txt
```
...
```
1.txt           3.txt           5.txt           AA.txt           CC.txt           EE.txt           Letters         myFirstFilEE.txt
...
```
ls  
cp myFirstFilEE.txt mySecondFilEE.txt
ls
```
You can verify it is a copy by `cat`ing the file, or you can use `more` or `less`
```
cat myFirstFilEE.txt
cat mySecondFilEE.txt
```
...
```
ls -l myFirstFilEE.txt mySecondFilEE.txt
```
...
**Exercise:** Identify the 5 most used words in myFirstFilEE.txt.  (Hint: cat,tr,sort, uniq, sort, tail, \|)

```
cat myFirstFilEE.txt  | tr ' ' '\n' | sort | uniq -c | sort -n | tail -n 4
  4 is
  4 space
  5 the
  5 to
